...
public class Server {
    ...
    static final Object lock = new Object(); // 락 객체
                ...
                Thread clientThread = new Thread(() -> { // 새 클라이언트에 새 쓰레드 할당
                    try {
                        ...
                        synchronized (readyPlayer) {
                            ...
                            if (readyPlayer.get(currentClientNo) == null || readyPlayer.containsValue(null)) {// 두 클라이언트가 준비되지 않은 경우
                        ...
                        synchronized (category) {
                            if (oppNum > currentClientNo) { // 한쪽에서만 카테고리 합치기
                        ...
                        if (ok.equals("ok")) { // ok싸인이 제대로 왔으면
                            ...
                            for (int i = 0; i < 10; i++) {
                                String pname;
                                synchronized (category) {
                                    pname = category.peopleList.get(i).name; // 이름(답)
